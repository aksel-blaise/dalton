...
Of those landmark and semilandmark configurations included in the draft sketches for this project, it is sLMs 12 and 13 that appear to contribute least to the formal tests of those hypotheses tested using EFA. They are, however, still included in the 3D landmarking protocol since the z-axis may provide additional useful data. For this reason, the same analysis is presented twice (Chapters 4 and 5). In Chapter 4, sLMs 12 and 13 are included, but in Chapter 5 they are not. Both landmarking configurations are analysed using the `LaSEC` function in the `LAMBDA` package in the next chapter.
...
```{r lm3d3.landmark, out.width = "100%", dpi = 300, echo=TRUE, warning=FALSE, fig.cap="_**Figure 1. Coordinates of landmarks populated using the [LM3d1](https://github.com/aksel-blaise/gahaganmorph2/blob/master/analysis/landmarking-protocol.md) protocol serve as the basis for LM3d3.**_"}
