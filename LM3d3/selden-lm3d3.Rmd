...
A suite of visual aids were created as a means of visualising the hypotheses that LM3d3:c1 will be used to test. The visual representation of the following hypotheses were produced for my own reference, and were used to critically assess the utility of the semilandmarks employed in the landmarking protocol. _All hypotheses were assessed using the same sample of Dalton projectile points._

### Hypothesis 1

Hypothesis 1 will assess whether there is a difference in morphology for Dalton points found in and out of the heartland.
...
Hypothesis 2 will assess whether there is a difference in morphology for Dalton points found in the heartland, the interior, and the northern periphery.
...
Hypothesis 3 will assess whether there is a discernible difference in morphology for Dalton points that are beveled, or not beveled.
...
LM3d3: Configuration 2 (LM3d3:c2) is an extension and subset of LM3d3:c1, and is used in the assessment of Hypothesis 4. Following the same methods described above, three additional equidistant sections were placed between LM 01 and the first cross section on the blade, and were inserted using ref.pl.4 as the base plane.

```{r lm3d3.c2.section, out.width = "100%", dpi = 300, echo=TRUE, warning=FALSE, fig.cap="_**Figure 22. Five equidistant sections inserted between LM 01 and the first cross section. Those sections located at LM 01 and the first cross-section were deleted.**_"}
knitr::include_graphics('images/lm3d3.c2.section.png')
```

Each of the sections was split at the points of highest curvature along the lateral edges, then at the intersection of the curves and ref.pl.1.

```{r lm3d3.c2.split, out.width = "100%", dpi = 300, echo=TRUE, warning=FALSE, fig.cap="_**Figure 23. Curves were split at the points of highest curvature along the lateral edges, then at the intersection of the curves and ref.pl.1.**_"}
knitr::include_graphics('images/lm3d3.c2.geom.png')
```

Semilandmarks were then populated in following with the preceding protocol, where sLMs 31 through 33 were placed on the right side of the projectile, and sLMs 34 through 36 were placed on the left. Semilandmarks 37 through 42 were placed along the intersection of the sections and ref.pl.1., and sLMs 43 through 54 were placed along the curves equidistant between those on the lateral edge and ref.pl.1. 

```{r lm3d3.c2.slm, out.width = "100%", dpi = 300, echo=TRUE, warning=FALSE, fig.cap="_**Figure 24. Location of semilandmarks added to address Hypothesis 4.**_"}
