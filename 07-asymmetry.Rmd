# Bilateral/Object Asymmetry

In studies of projectile morphology, an analysis of bilateral/object symmetry can yield useful insights related to _design intent_, provided the assumption that the goal of the knapper was to achieve a fully-symmetrical form. Within that framework, deviation from symmetry (asymmetry) can serve as a useful measure of _knapper skill_. Both measures have utility in comparisons of contexts and sites, to include wide-ranging spatial and temporal applications. Initially developed for a [symmetry analysis of Gahagan bifaces](https://github.com/aksel-blaise/gahagan.asymmetry/blob/master/asymmetry.md), the same approach can be employed for an analysis of symmetry in Dalton projectile points.
...
```{r pca.asymm.1, out.width = "100%", dpi = 300, echo=TRUE, warning=FALSE}
# plot pca by heartland in (H) and out (N)
pc.plot.asymm.1 <- plot(pca.asymm, 
                 asp = 1,
                 pch = pch.gps.heart,
                 col = col.gps.heart)
                    shapeHulls(pc.plot.asymm.1, 
                             groups = heart,
                             group.cols = col.hull)
```
...
```{r pca.asymm.2, out.width = "100%", dpi = 300, echo=TRUE, warning=FALSE}
# plot pca by heartland + regions (asymmetric)
pc.plot.asymm.2 <- plot(pca.asymm, 
                   asp = 1,
                   pch = pch.gps.hreg,
                   col = col.gps.hreg)
                    shapeHulls(pc.plot.asymm.2, 
                               groups = hreg,
                               group.cols = col.hull.2)
```
...
```{r pca.asymm.3, out.width = "100%", dpi = 300, echo=TRUE, warning=FALSE}
# plot pca by heartland in (H) and out (N) (asymmetric)
pc.plot.asymm.3 <- plot(pca.asymm, 
                   asp = 1,
                   pch = pch.gps.bev.1,
                   col = col.gps.bev.1)
                      shapeHulls(pc.plot.asymm.3, 
                               groups = bev.1,
                               group.cols = col.hull.3)
```
...
```{r pca.asymm.4, out.width = "100%", dpi = 300, echo=TRUE, warning=FALSE}
# plot pca by bevel type (asymmetric)
pc.plot.asymm.4 <- plot(pca.asymm, 
                   asp = 1,
                   pch = pch.gps.bev.2,
                   col = col.gps.bev.2)
                      shapeHulls(pc.plot.asymm.4, 
                               groups = bev.2,
                               group.cols = col.hull.4)
```
